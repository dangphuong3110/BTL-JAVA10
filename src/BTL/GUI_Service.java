...
public class GUI_Service extends JFrame {
	...
	public void findService_byID(String idService) {
		if (tf_idService.getText().equals("")) {
			JOptionPane.showConfirmDialog(null, "Vui lòng nhập ID khách hàng cần tìm!", "Thông báo",
			...
			if (jOP_Check.showConfirmDialog(null, "Bạn có tìm kiếm dịch vụ theo mã này?", "Tìm kiếm khách hàng",
	...
	public void insertService() {
		if (tf_idService.getText().equals("") || tf_nameService.getText().equals("")
			...
			JOptionPane.showConfirmDialog(null, "Vui lòng nhập đủ thông tin dịch vụ!", "Thông báo",
					JOptionPane.CLOSED_OPTION);
		else {
			...
			if (jOP_Check.showConfirmDialog(null, "Bạn có muốn thêm dịch vụ này?", "Thêm dịch vụ",
					jOP_Check.YES_NO_CANCEL_OPTION) == 0) {
				if (pp.insertService(idService, nameService, priceService)) {
					...
					tf_nameService.setText("");
					tf_priceService.setText("");
					jOP_Message.showMessageDialog(null, "Thêm dịch vụ thành công!", "Thông báo",
							JOptionPane.INFORMATION_MESSAGE);
				} else
					jOP_Message.showMessageDialog(null, "Mã dịch vụ bị trùng!", "Thông báo",
							JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
	...
	public void updateService() {
		if (tf_idService.getText().equals("") || tf_nameService.getText().equals("")
			...
			String nameService = tf_nameService.getText();
			int priceService = Integer.parseInt(tf_priceService.getText());
			if (jOP_Check.showConfirmDialog(null, "Bạn có muốn cập nhật thông tin dịch vụ này?", "Thêm dịch vụ",
					jOP_Check.YES_NO_CANCEL_OPTION) == 0) {
				if (pp.updateService(idService, nameService, priceService)) {
					...
					tf_nameService.setText("");
					tf_priceService.setText("");
					jOP_Message.showMessageDialog(null, "Cập nhật thông tin dịch vụ thành công!", "Thông báo",
							JOptionPane.INFORMATION_MESSAGE);
				} else
					jOP_Message.showMessageDialog(null, "Cập nhật thông tin dịch vụ không thành công!", "Thông báo",
							JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
	...
	public void deleteService() {
		...
		jOP_Message.showMessageDialog(null,
				"Không khả dụng! Xóa dịch vụ sẽ làm ảnh hưởng rất nhiều tới dữ liệu của phần mềm. Chọn thêm hoặc sửa để sử dụng!!",
				"Thông báo", JOptionPane.INFORMATION_MESSAGE);
	}

	public void clearInput() {
		if (jOP_Check.showConfirmDialog(null, "Bạn có muốn xóa tất cả ô nhập dữ liệu không?", "Xóa ô nhập",
				jOP_Check.YES_NO_CANCEL_OPTION) == 0) {
			getListService();
			tf_idService.setText("");
			tf_idService.enable(true);
			tf_nameService.setText("");
			tf_priceService.setText("");
			jOP_Message.showMessageDialog(null, "Xóa ô nhập dữ liệu thành công!", "Thông báo",
					JOptionPane.INFORMATION_MESSAGE);
		}
	}
